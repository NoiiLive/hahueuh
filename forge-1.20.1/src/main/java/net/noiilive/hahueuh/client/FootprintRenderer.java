...
public final class FootprintRenderer {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        List<FootprintSyncPacket.Footprint> footprints = ClientFootprintState.footprints();
        ...
        for (FootprintSyncPacket.Footprint f : footprints) {
            ...
            part.render(pose, buffer, LightTexture.FULL_BRIGHT, OverlayTexture.NO_OVERLAY, cr, cg, cb, ca);
            pose.popPose();
        }
    ...
    private static void updateHover(Minecraft mc, float pt, List<FootprintSyncPacket.Footprint> footprints) {
        Vec3 eye = mc.player.getEyePosition(pt);
        Vec3 look = mc.player.getViewVector(pt);

        FootprintSyncPacket.Footprint best = null;
