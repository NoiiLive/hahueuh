...
public final class FingerHighlightRenderer {
    private FingerHighlightRenderer() {}

    private static final int R = 0xB6;
    private static final int G = 0x29;
    private static final int B = 0xFC;
    private static final int A = 255;

    @SubscribeEvent
    public static void onRenderLevel(RenderLevelStageEvent event) {
        if (event.getStage() != RenderLevelStageEvent.Stage.AFTER_ENTITIES) return;
        if (ClientFingerHighlightState.isEmpty()) return;
        ...
        for (int id : ClientFingerHighlightState.highlighted()) {
