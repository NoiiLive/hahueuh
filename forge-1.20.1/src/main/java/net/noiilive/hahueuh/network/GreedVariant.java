...
public enum GreedVariant {
    ...
    public static GreedVariant randomWeighted(RandomSource random) {
        int wLion = Math.max(0, ConfigGreed.VARIANT_WEIGHT_LIONSHEART.get());
        int wCor = Math.max(0, ConfigGreed.VARIANT_WEIGHT_CORLEONIS.get());
        int wEchidna = Math.max(0, ConfigGreed.VARIANT_WEIGHT_ECHIDNA.get());
