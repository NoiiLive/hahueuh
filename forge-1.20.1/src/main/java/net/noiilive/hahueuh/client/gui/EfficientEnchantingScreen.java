...
import net.noiilive.hahueuh.network.EfficientEnchantOptionsData;
import net.noiilive.hahueuh.menu.EfficientEnchantingMenu;
import net.noiilive.hahueuh.network.BackToEnchantingPacket;
import net.noiilive.hahueuh.network.EfficientEnchantOptionsPacket;
import net.noiilive.hahueuh.network.EfficientEnchantSelectPacket;
...
public final class EfficientEnchantingScreen extends AbstractContainerScreen<EfficientEnchantingMenu> {
    ...
    private List<EfficientEnchantOptionsPacket.Option> options() {
    ...
    private void renderBook(GuiGraphics g, int x, int y, float partialTick) {
        ...
        bookModel.renderToBuffer(g.pose(), buffer, 15728880, OverlayTexture.NO_OVERLAY, 1.0f, 1.0f, 1.0f, 1.0f);
        g.flush();
        g.pose().popPose();
    ...
    private void renderOptions(GuiGraphics g, int mouseX, int mouseY) {
        List<EfficientEnchantOptionsPacket.Option> opts = options();
        ...
        if (menu.itemSlot().isEmpty()) {
            ...
            for (int row = 0; row < VISIBLE_ROWS; row++) {
                ...
                EfficientEnchantOptionsPacket.Option opt = opts.get(idx);
                int rx = listLeft;
    ...
    private Component enchantName(EfficientEnchantOptionsPacket.Option opt) {
        Enchantment enchantment = enchantmentFor(opt.enchantmentId());
        if (enchantment == null) return Component.literal(opt.enchantmentId());
        return enchantment.getFullname(opt.level());
    }

    private Enchantment enchantmentFor(String id) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isHovering(BACK_BTN_X, BACK_BTN_Y, BACK_BTN_SIZE, BACK_BTN_SIZE, mouseX, mouseY)) {
            ModNetworking.CHANNEL.sendToServer(new BackToEnchantingPacket());
            playClickSound();
            return true;
        }
        ...
        if (!menu.itemSlot().isEmpty() && canAfford()) {
            ...
            for (int row = 0; row < VISIBLE_ROWS; row++) {
                ...
                if (isHovering(LIST_X, LIST_Y + row * ROW_H, ROW_W, ROW_H, mouseX, mouseY)) {
                    ModNetworking.CHANNEL.sendToServer(new EfficientEnchantSelectPacket(opts.get(idx).enchantmentId()));
                    playClickSound();
                    return true;
                }
            }
